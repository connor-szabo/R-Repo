...
while (c < 1000) {
  ...
  if (ta < t) {      # arrival event is triggered
    ...
    if(n == 0 & s == 0) { # When server is free (queue should be empty)
      ...
      bs <- t.clock
      wt[i] = 0      #If the process is put directly into the server then there is no wait time
    }
    else{ # Server is busy (n should be > 0)
      n <- n + 1 # increase the size of queue (system state)
      wt[i] <- t.clock   #If the process is put in the queue, record the time it entered at
      if(n > maxQueue){
        maxQueue <- n
      }
    }
    ...
    if (n > 0) { # queue is not empty
      ...
      wt[i] <- t.clock - wt[i]   #Find the amount of time waiting for the process being put into the server
